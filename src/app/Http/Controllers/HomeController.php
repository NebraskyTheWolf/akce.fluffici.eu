...
class HomeController extends Controller
{
    ...
    public function akce(Request $request): View|Application|Factory|\Illuminate\Contracts\Foundation\Application
    {
        ...
        if ($event->startAt === null) {
            ...
            $event->startAt = $start->monthName . ' ' . $start->day . ', ' . $start->year;
            $event->startAtTime = $start->hour . ':' . $start->minute;

            $end = Carbon::parse($event->end);
            $event->endAt = $end->monthName . ' ' . $end->day . ', ' . $end->year . ' ' . $end->hour . ':' . $end->minute;
        }

        if ($event->banner_url === null) {
            if ($event->banner_id != null) {
                $event->banner_url = "https://autumn.fluffici.eu/banners/" . $event->banner_id . "?width=800&height=400";
            } else {
                $event->banner_url = "https://placehold.co/800x400";
            }
        }

        if ($event->map_url === null) {
            if ($event->map_id != null) {
    ...
    public function submittedPictures(Request $request): View|Application|Factory|\Illuminate\Contracts\Foundation\Application {
        $pictures = EventAttachments::where('user_id', $request->user()->id)
            ->where('published', true)
            ->orderby('created_at', 'desc')
            ->get();

        return view('layouts.submitted-pictures', compact('pictures'));
    }

    public function submittedReports(Request $request): View|Application|Factory|\Illuminate\Contracts\Foundation\Application {
