        ...
        @if(Subscriptions::where('user_id', Auth::id())->exists())
            @if(Subscriptions::where('user_id', Auth::id())->first()->is_subscribed === 1)
                <a class="subscribe" href="{{ route('notification.subscribes') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="bell-icon" width="36" height="36" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff" fill="none" stroke-linecap="round" stroke-linejoin="round" style="display:initial">
